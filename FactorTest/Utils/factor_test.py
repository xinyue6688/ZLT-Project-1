...
class FactorDecileAnalysis:
    ...
    def __init__(self, df, group_num):
        """
        初始化因子分析类

        :param df: 数据框，需包含以下列：'TRADE_DT'（交易日期）,
                                     'WIND_PRI_IND'（行业代码）,
                                     'turnover'（换手率）,
    ...
    def industry_neutralize_and_group(self):
        """
        对数据进行行业中性化及分组
        ...
        for (date, industry), group in grouped:
            ...
            try:
                ...
                df.loc[group.index, 'decile'] = pd.qcut(group['turnover'], q, labels=labels,
